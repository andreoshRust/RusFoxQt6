...
RusFoxQt6 — это мощный русскоязычный 1С-подобный язык программирования для создания сценариев автоматизации, написанный на C++ с использованием библиотеки Qt6. Это расширение и руссификация интерпретатора JavaScript (QJSEngine), встроенного в библиотеку Qt6. Не знаете, как написать код на русском, можете писать на JavaScript. В итоге препроцессор всё равно для исполнения создаёт файл на JavaScript, который может запускаться с директивой отключения препроцессора. 

Язык позволяет быстро создавать скрипты для работы с файлами, базами данных, сетевыми запросами, графикой, печатью и многим другим. Мой предыдущий проект RusFox (руссификация и расширение FoxPro9) считаю уже устаревшим и не планирую развивать дальше. 
Нейросистемы успешно пишут сценарии для RusFoxQt6. Достаточно показать им образцы сценариев и руководство к программе, в которой около 1000 страниц описания языка с примерами. Руссификация сценариев делает код понятным, а потому позволяет легко поправить код после нейросистемы. Например, интерфейс программы часто проще поправить самому после размещения виджетов в окне нейросистемой, чем объяснять нейросистеме, куда передвинуть и как оформить кнопку. 

Процесс создания сценариев совместно с бесплатным DeepSeek выглядит так:
Вы пишите многооконное большое приложение, но лучше писать его по частям по одному окну с функционалом. Показываете ИИ-агенту описание (просто копируете документацию около 1000 страниц в окно чата), затем пример создания окна с виджетами из сценария 019_Образец_1Окно.RUS  , затем показываете сценарий с примером подобного или близкого функционала и просите реализовать свою идею в одном окне. Копируете в чат сообщения об ошибках и лог программы, чтоб ИИ-агент сам исправил ошибки. Затем, протестировав функционал в одном окне, добавляете окно в свой многооконный сценарий.
Далее можно просить Deepseek создавать только функцию, в которой создается окно с новым функционалом, но при смене агента (в новом чате) опять его надо обучить, показав примеры.

ИИ-Агент Гермес подключенный к DeepSeek вполне успешно обучается писать сценарии и тесты для сценария. Сам создает файл сценария, запускает его, смотрит результат и исправляет. В будущем планирую выкладывать сюда настройки для ИИ-агента по написанию сценариев.

Саму программу RusFoxQt6 v2.3 можно взять здесь   [https://transfiles.ru/5cqke](https://transfiles.ru/5cqke)   (ссылка действует до 24.06.2026)  Если у вас была старая версия RusFoxQt6 на компе, то надо разархивировать новую версию и перед запуском новой версии надо очистить кэш, запустив newVersionRusFoxQt6.bat . После этого загрузчик развернёт во временном каталоге новые файлы для запуска.
